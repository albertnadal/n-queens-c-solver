...
void solve_n_queens(int start_x, int end_x, int y, uint32_t vertical_mask, uint32_t left_diagonal_mask, uint32_t right_diagonal_mask, int thread_idx, thread_data *threads_data) {
  if (y >= N) {
#if STORE_SOLUTIONS
    // Store the solution in the solutions array
    for (int i = 0; i < 2 * N; i += 2) {
      threads_data[thread_idx].solutions[threads_data[thread_idx].solutions_count][i] = threads_data[thread_idx].current_solution[i];
      threads_data[thread_idx].solutions[threads_data[thread_idx].solutions_count][i + 1] = threads_data[thread_idx].current_solution[i + 1];
      threads_data[thread_idx].solutions[threads_data[thread_idx].solutions_count + 1][i] = threads_data[thread_idx].current_solution[i];
      threads_data[thread_idx].solutions[threads_data[thread_idx].solutions_count + 1][i + 1] = (N - 1) - threads_data[thread_idx].current_solution[i + 1];
    }
#endif
    threads_data[thread_idx].solutions_count += 2;
    return;
  }

  for (int x = start_x; x < end_x; x++) {
    if (is_valid_queen(x, y, vertical_mask, left_diagonal_mask, right_diagonal_mask)) {
      threads_data[thread_idx].current_solution[y * 2] = x;
      threads_data[thread_idx].current_solution[y * 2 + 1] = y;
      solve_n_queens(0, N, y + 1, vertical_mask | (1U << x), left_diagonal_mask | (1U << (x + y)), right_diagonal_mask | (1U << (x - y + N - 1)), thread_idx, threads_data);
    }
  }
}
...
int main(int argc, char *argv[]) {
  ...
  thread_data *threads_data = calloc(max_usable_threads, sizeof(thread_data));
  printf("Solving %d-Queen Problem with %d parallel threads.\n", N, max_usable_threads);
